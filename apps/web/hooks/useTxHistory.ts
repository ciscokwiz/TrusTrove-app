...
async function fetchPage(address: string, cursor?: string) {
  ...
  let txQuery = server.transactions()
    .forAccount(address)
    .limit(10)
    .order('desc');
  ...
  for (let i = 0; i < txPage.records.length; i++) {
    ...
    const matchingOps = ops.filter((op: any) =>
      op.contract_id && CONTRACT_IDS.includes(op.contract_id)
    );
  ...
  const nextCursor = txPage.records.length === 10 ? lastRecord.paging_token : null;
